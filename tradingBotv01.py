...
class TradingBot:
    def __init__(self, symbol, amortized_investment, rate, csv_file='data.csv', sleep_time=60):
        """
        ...
        self.current_investment, self.fixed_capital  = self._get_blance_info()
        print(f"Initial Balances - {self.symbol[:-4]}: {self.current_investment}, USDT: {self.fixed_capital}")
    ...
    def _get_blance_info(self, quote_asset='USDT'):
        """Fetches and returns the balance information for the trading pair."""
        try:
            base_asset = self.symbol[:-len(quote_asset)]  # Assuming symbol ends with 'USDT'
            base_balance = self.client.get_asset_balance(asset=base_asset)
            quote_balance = self.client.get_asset_balance(asset=quote_asset)

            ticker_info = self.client.get_symbol_ticker(symbol=self.symbol)
            symbol_price_equivalent = float(ticker_info['price'])

            # Calculate the total USDT value
            total_usdt_value = (float(base_balance['free'])) * symbol_price_equivalent

            return float(total_usdt_value), float(quote_balance['free'])
        except BinanceAPIException as e:
            logging.error(f"API Error getting balance info: {e}")
            return 0.0, 0.0
        except Exception as e:
            logging.error(f"General error getting balance info: {e}")
            return 0.0, 0.0
    ...
    def _get_quantity_to_trade(self, info, current_price, investment_amount):
        ...
        return round(quantity_to_buy, 7), round(order_value, 7)
    ...
    def run_bot(self):
        ...
        while True:
            ...
            self.current_investment, self.fixed_capital  = self._get_blance_info() # Update balances
...
if __name__ == "__main__":
    ...
    SLEEP_TIME = os.getenv("SLEEP_TIME") # In seconds
